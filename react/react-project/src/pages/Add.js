...
class Add extends React.Component {
    ...
    deleteUser(e) {
        axios.delete(`https://64b53279f3dbab5a95c6e9e2.mockapi.io/api/v1/user/${e.currentTarget.getAttribute("data-value")}`)
            .then(res => {
                if (res.status === 200) {
    ...
    renderUser() {
        ...
        return users['person'].map(({ id, firstname, lastname, email, password, avatar }) => {
            return <tr key={id} >
                ...
                <td>
                    <div className="row">
                    <button className="col-4 form-control btnDel btn-success" onClick={this.deleteUser} data-value={id}>delete</button>
                    <button className="col-4  form-control btnDel btn-success" onClick={this.editUser} data-value={id}>
                        edit
                    </button>
                    </div>
